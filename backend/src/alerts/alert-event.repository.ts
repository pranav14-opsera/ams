...
import type { AlertEvent, AlertMetricName, AlertSeverity } from "./alert-threshold.types";
...
export class AlertEventRepository {
  ...
  async create(
    ...
    tenantId: string,
    agentId: string,
    fields: { metricName: AlertMetricName; thresholdValue: number; actualValue: number; severity: AlertSeverity; breachTimestamp: Date },
  ): Promise<AlertEvent> {
    const executor = client ?? this.pool;
    const result = await executor.query<AlertEventRow>(
      `INSERT INTO alert_events (tenant_id, agent_id, metric_name, threshold_value, actual_value, severity, breach_timestamp)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`,
      [tenantId, agentId, fields.metricName, fields.thresholdValue, fields.actualValue, fields.severity, fields.breachTimestamp],
    );
    return toDomain(result.rows[0]);
  }

  /** Cooldown check: the most recent alert for this exact agent+metric, or null if none has ever fired. */
  async findMostRecent(client: Pool | PoolClient | undefined, tenantId: string, agentId: string, metricName: AlertMetricName): Promise<AlertEvent | null> {
    const executor = client ?? this.pool;
    const result = await executor.query<AlertEventRow>(
